# Fix — Staff photo upload silently fails for normal-sized photos (verified against live codebase 15 June 2026; re-verified still open 18 June 2026)
...
1. The server caps request bodies at **5MB** — `app.use(express.json({ limit: '5mb' }))` in `server/index.ts:246` (and the matching `urlencoded` on line 247).
...
Even though resizing makes huge bodies impossible from this screen, raise the limit a little so other base64 upload paths (logos, documents) are also more forgiving. In `server/index.ts:246-247`:
