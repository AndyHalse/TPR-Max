...
These 500s were seen by **Andy solo-testing a demo account** (a likely cold/suspended database), not a real customer under load. TPR's architecture is **intentionally one database per site** (e.g. Cowiesburn = 120 sites, 120 databases) — that's correct and not up for change. So the pool finding below is **not a go-live blocker**; it's an optional resilience improvement. It only bites if a *single site* ever has several people using TPR at the same moment, or a live mustering screen during an evacuation. Treat as "nice hardening", and confirm the 500s weren't simply a Neon cold-start before doing major work.

## LIKELY MECHANISM (found by code review)
...
`max: 1` is fine for a single user, and fine for the one-database-per-site model when each site is light-use. It only becomes a problem if a single site has concurrent users or a busy/mustering screen. So this is a resilience tweak, not an emergency.
