...
Because this one is broader and higher-risk, it's been split into its own prompt — see `bugfix-objects-no-access-control-june2026.md`, which covers the root cause (the serve route checks nothing **and** no upload ever tags a file public/private) and a two-phase fix that closes the hole without breaking logos. Do that one separately; don't fold it into these portal fixes.
