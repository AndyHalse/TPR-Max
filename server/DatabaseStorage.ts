...
export class DatabaseStorage implements IStorage {
  ...
  async updateStaff(id: string, updates: Partial<InsertStaff>): Promise<Staff | undefined> {
    ...
    if (existingStaff) {
      ...
      // 2. Already is a Fire Marshal (existingStaff.isFireMarshal === true)
      // AND they don't have a URL ID yet
      const isOrWillBeFireMarshal = updates.isFireMarshal === true || existingStaff.isFireMarshal === true;
      const hasNoUrlId = !existingStaff.fireMarshalUrlId && !updates.fireMarshalUrlId;
  ...
  async getReceptionDiaryByCustomer(customerId: string, startDate: Date, daysAhead: number): Promise<any[]> {
    ...
    const diaryEntries = await db.select({
      ...
      customerId: preBookings.customerId,
      // Host staff details
