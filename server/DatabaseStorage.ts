...
export class DatabaseStorage implements IStorage {
  ...
  private async initializeSampleDataIfEmpty(): Promise<void> {
    try {
      ...
      console.log('🏗️ Initializing sample data for fresh database...');
  ...
  private async createSampleData(): Promise<void> {
    ...
    const sampleVisitors = [
      {
        ...
        hostStaffId: sampleStaff[0].id,
        isCheckedIn: true,
        checkedInAt: new Date(),
        checkedOutAt: null,
        qrCode: `visitor-${randomUUID()}`,
      },
      {
        id: randomUUID(),
        name: "Lisa Anderson", 
    ...
    // Insert visitors
    await db.insert(visitors).values(sampleVisitors);

    // Create sample pre-bookings
    ...
    const samplePreBookings = [
      {
        id: randomUUID(),
        visitorName: "David Williams",
        visitorEmail: "david.williams@supplier.com",
        company: "Supplier Corp",
        purpose: "Quarterly review meeting",
        visitDate: tomorrow,
        visitTime: "10:00",
        hostStaffId: sampleStaff[0].id,
        qrCode: `prebooking-${randomUUID()}`,
    ...
    // Insert pre-bookings
    await db.insert(preBookings).values(samplePreBookings);
