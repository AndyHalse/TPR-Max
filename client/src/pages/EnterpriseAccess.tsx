...
export default function EnterpriseAccess() {
  ...
  const { data: myGrants, isLoading: myGrantsLoading, isError: myGrantsError, error: myGrantsErrorObj, refetch: refetchMyGrants } = useQuery<MyGrants>({
  ...
  if (myGrantsError) {
    const is403 = (myGrantsErrorObj as any)?.status === 403;
    return (
      <div className="p-6 flex items-center justify-center min-h-64">
        <div className="p-8 max-w-sm text-center space-y-3 border rounded-xl bg-card">
          ...
          <h2 className="font-semibold">{is403 ? "Access restricted" : "Couldn't load access settings"}</h2>
          ...
          {!is403 && (
            <Button variant="outline" size="sm" onClick={() => refetchMyGrants()}>Try again</Button>
          )}
        </div>
      </div>
    );
