...
export default function ContractorKiosk() {
  ...
  const scanFrame = useCallback(() => {
    ...
    // Guard: videoWidth > 0 is the reliable live-stream indicator.
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      // Wait for the camera to produce its first frame. 'playing'/'canplay'
      // fires once the browser has decoded the first frame. 4 s timeout
      // covers slow USB cameras that delay before producing data.
      await Promise.race([
      ...
      // Only show the viewfinder if the stream is still active (user may have
