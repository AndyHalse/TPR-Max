...
- **Additional anchors confirmed during this scan:** `server/routes/passes.ts` public QR scan endpoints, `server/qrReaderService.ts` cross-tenant QR lookup logic, `server/routes/visitors.ts` XStation scanner flow, H&S acceptance links, and Paxton integration routes, `server/routes/calendarIntegration.ts` OAuth callbacks, `server/objectStorage.ts` legacy object-path normalization, `server/routes/contractors.ts` public contractor document-request uploads, `server/routes/induction.ts` BioStar webhook and sync routes, `server/routes/reports.ts` report export/email endpoints, `server/routes/aiFeatures.ts` paid image-generation endpoint, `server/routes/enterpriseSites.ts` active-site selection, `server/siteScope.ts` site-scope enforcement
