...
- **AI-Powered Induction Video System**: Production-ready safety training video generation using OpenAI's advanced AI stack. GPT-5 generates context-aware safety scripts tailored to company requirements. DALL-E 3 creates photorealistic, scene-specific workplace safety images using contextual prompts (title + description) instead of generic templates. OpenAI TTS provides professional voice narration with role-specific voices (alloy/onyx/nova). Fallback chain ensures reliability: DALL-E 3 → Gemini → SVG. Both DALL-E 3 and Gemini use contextual prompts to generate images relevant to each specific scene, eliminating generic "high-vis jacket" defaults.
- **CO2 Sustainability Reporting System**: Comprehensive carbon footprint tracking for contractor commutes using Gemini AI (via Replit AI Integrations). Gemini-2.0-flash-exp calculates distances between UK postcodes with intelligent route type detection (motorway/A-roads/mixed). Generates detailed sustainability reports analyzing total emissions, worker breakdowns, and actionable recommendations. All reports are multi-tenant isolated with database storage. Fallback logic ensures reliability when AI is unavailable. Billed to Replit credits with no API key management required.
