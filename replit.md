...
- **Compliance Dashboard** (`/compliance`): Admin-only page showing 8 Martyn's Law requirements mapped to live TPR Max system data. Features: compliance score percentage with colour-coded progress bar, per-requirement rows with green tick / amber "Action needed" badges, "What is Martyn's Law?" accordion with official Home Office factsheet link, "Download Compliance Summary" PDF button (`GET /api/compliance/report` — Puppeteer PDF with HTML fallback). Backend `GET /api/compliance/summary` returns structured per-requirement JSON from live data (zones, fire marshals, drills, preBookings, incidentReports, companySettings). Sidebar entry visible to admins only (`adminOnly: true`).
