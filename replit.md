...
- **Martyn's Law (UK Protect Duty)**: Dedicated compliance section at `/martyn-law` with 4 tabs: Venue & Scope, 12-item UK compliance Checklist, Security Plan (terrorism action plan / lockdown / evacuation / comms), and an Evidence Log (training/drills). Customer-isolated storage in `martyn_law_config` table (migration #024). Navigation link added to sidebar. Includes "What is Martyn's Law?" collapsible accordion, "Download Report" button (opens printable HTML via `GET /api/compliance/report`), and `GET /api/compliance/summary` JSON endpoint for programmatic compliance score retrieval.
